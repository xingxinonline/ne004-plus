...
{
    APP_LOGE("ERROR", "Error 0x%08X: %s", error_code, error_msg ? error_msg : "Unknown error");
...
/* 函数声明 */
static int app_parse_command_internal(const char* input, char* cmd, char* args);
 ...
 * @brief 错误处理函数
 */
void app_error_handler(uint32_t error_code, const char* error_msg)
{
    APP_LOGE("ERROR", "Error 0x%08lX: %s", error_code, error_msg ? error_msg : "Unknown error");
...
static void app_process_input(void)
{
    ...
    if (command_ready) {
        ...
        /* 解析命令 */
        if (app_parse_command_internal(input_buffer, cmd, args) == 0) {
...
{
    if (!cmd_line) return APP_CMD_UNKNOWN;
    
    /* 简化实现 - 直接从命令行字符串获取命令类型 */
    char cmd[32] = {0};
    int i = 0;
    ...
    /* 提取第一个单词作为命令 */
    while (*cmd_line && *cmd_line != ' ' && *cmd_line != '\t' && i < 31) {
        cmd[i++] = *cmd_line++;
    }
    cmd[i] = '\0';
...
{
    app_handle_command(cmd, args);
    return 0;
}

/**
 * @brief 显示帮助信息 (头文件要求的接口)
 */
