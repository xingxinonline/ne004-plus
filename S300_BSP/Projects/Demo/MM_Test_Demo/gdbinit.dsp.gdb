...
# MM Test Demo debug init with DSP boot images
# Usage:
#   arm-none-eabi-gdb -q -ex "file <path-to>/s300_mm_test_demo" -ex "target extended-remote :3333" -x S300_BSP/Projects/Demo/MM_Test_Demo/gdbinit.dsp.gdb
...
# Images are in MM_Test_Demo/DSP_Images
# GDB is launched from MM_Test_Demo/build
...
shell if [ -s "../DSP_Images/dsp_dtcm_boot.bin" ]; then echo "restore ../DSP_Images/dsp_dtcm_boot.bin binary 0x44800000" >> restore_imgs.tmp; fi
shell if [ -s "../DSP_Images/dsp_ptcm_boot.bin" ]; then echo "restore ../DSP_Images/dsp_ptcm_boot.bin binary 0x44A00000" >> restore_imgs.tmp; fi
shell if [ -s "../DSP_Images/dsp_sram0_boot.bin" ]; then echo "restore ../DSP_Images/dsp_sram0_boot.bin binary 0x44000000" >> restore_imgs.tmp; fi

source restore_imgs.tmp
shell rm restore_imgs.tmp

# Go!
echo \n>>> Starting MM Test demo with DSP boot...\n
...
commands
  ...
  shell if [ -s "../DSP_Images/dsp_psram_boot.bin" ]; then echo "restore ../DSP_Images/dsp_psram_boot.bin binary 0x80000000" >> restore_psram.tmp; fi
